...
@interface AddBorrowVC ()<NIDropDownDelegate,QBImagePickerControllerDelegate,UIImagePickerControllerDelegate>
{
    NIDropDown *dropDown;
}
...
@property (strong, nonatomic) IBOutlet MSTextField *txtBorrowedBy;
...
@property (strong, nonatomic) IBOutlet UIButton *btnLendBy;
...
- (IBAction)btnAddTapped:(id)sender{
    if ([self isValidDetails]) {
        @try
        {
            ...
            NSDictionary *parameters  = @{@"userid":[NSNumber numberWithInteger:[User sharedUser].intUserId],@"name":_txtItemName.text,@"categoryid":[NSNumber numberWithInteger:_intCatID],@"itemuserid":[NSNumber numberWithInteger:_intFriendID],@"returndate":returnDate,@"itemtype":@2,@"itemphotocount":[NSNumber numberWithInteger:_arrImages.count]};
...
- (IBAction)btnLendByTapped:(id)sender{
    [_txtItemName resignFirstResponder];
    [self btnCloseTapped:nil];

    _btnCategory.selected = NO;
    _btnLendBy.selected = YES;
    if(dropDown == nil) {
...
- (IBAction)btnHelpLendByTapped:(id)sender{
    [_txtItemName resignFirstResponder];
    [dropDown hideDropDown:nil];
    _lblMessage.text = @"SELECT YOUR FRIEND THAT YOU LENT THE ITEM FROM. IF THEY DO NOT APPEAR IN THE LIST NAVIGATE TO THE FRIENDS MENU AND ADD YOUR NEW FRIEND.";
    _guideView.hidden = NO;
}
...
- (void) niDropDownDelegateMethod: (NIDropDown *) sender {
    ...
    }else if (_btnLendBy.selected){
        _txtBorrowedBy.text = sender.strTitle;
...
- (BOOL)isValidDetails{
    if(!_txtItemName.text || [_txtItemName.text isEmptyString]){
        ...
        return NO;
    }else if (!_txtBorrowedBy.text || [_txtBorrowedBy.text isEmptyString]) {
